...
{
	private final CurrencyAmountView BOBAmountView;
	...
	private final CurrencyAmountView.Listener BOBAmountViewListener = new CurrencyAmountView.Listener()
	{
		@Override
		public void changed()
		{
			if (BOBAmountView.getAmount() != null)
	...
	public CurrencyCalculatorLink(final CurrencyAmountView BOBAmountView, final CurrencyAmountView localAmountView)
	{
		this.BOBAmountView = BOBAmountView;
		this.BOBAmountView.setListener(BOBAmountViewListener);
	...
	public Coin getAmount()
	{
		...
		{
			return (Coin) BOBAmountView.getAmount();
		}
	...
	private void update()
	{
		BOBAmountView.setEnabled(enabled);
		...
		if (exchangeRate != null)
		{
			...
			if (exchangeDirection)
			{
				final Coin BOBAmount = (Coin) BOBAmountView.getAmount();
				if (BOBAmount != null)
				{
					localAmountView.setAmount(null, false);
					localAmountView.setHint(exchangeRate.coinToFiat(BOBAmount));
					BOBAmountView.setHint(null);
				}
			}
			else
			{
				...
				{
					localAmountView.setHint(null);
					BOBAmountView.setAmount(null, false);
					try
					{
						BOBAmountView.setHint(exchangeRate.fiatToCoin(localAmount));
					}
	...
	{
		if (exchangeDirection)
			return BOBAmountView.getTextView();
		else
	...
	public void setBOBAmount(final Coin amount)
	{
		final Listener listener = this.listener;
		this.listener = null;

		BOBAmountView.setAmount(amount, true);
	...
	{
		BOBAmountView.setNextFocusId(nextFocusId);
